...
var // detrended variable (cf: Y = Y_trend/Z_t)
    ...
    I, U
    //Pk, Re, NW
    // If Re, NW are removed, corresponding measurement equations must be removed
    // Wage determination
    ...
    Y_obs, R_obs,  dP_obs, C_obs, Inv_obs, dPcf_obs, X_obs, dPi_obs, 
    S_obs, Ystar_obs, Rstar_obs, Pistar_obs, G_obs, dPx_obs,
    // FA-related observed variables: NN_obs, SP_obs, 
    //W_obs
...
//(6) re, nw, k, Pk, rk, u, i
// FA block: 
// variables c, r, w, k, n, mc, y, rk, i, 
// financial accelerator variables: re, nw, pk, u
exp(k)-(1-DELTA)*exp(k(-1))/exp(dZtr)
= exp(Zi)*exp(i)*(1-KAPPAi/2*(exp(i)*exp(dZtr)/exp(i(-1))-exp(GAMMAtr))^2);
...
// financial accelerator variables: ret, nwt, pkt, ut
...
// exp(rkt) = (GAMMA1+GAMMA2*(exp(ut)-1))/exp(Zi);

/*
exp(ret) = (exp(pkt)*(1-DELTA) + exp(rkt)*exp(ut)
...
exp(yt) = exp(ct) + exp(it)
          + KAPPAi/2*(exp(it)*exp(dZtr)/exp(it(-1))-exp(GAMMAtr))^2*exp(it);
...
//,mode_check
//filtered_vars,
nograph) Y_obs, R_obs,  dP_obs, C_obs, Inv_obs, dPcf_obs, X_obs, dPi_obs, 
